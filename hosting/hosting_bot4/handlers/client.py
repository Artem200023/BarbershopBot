...
async def cm_date(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
	async with state.proxy() as data:
		selected, date = await SimpleCalendar().process_selection(call, callback_data)#.booking_days()

		if selected :
			data['date'] = date.strftime("%Y-%m-%d")
			#print(date['item'])
			await call.message.answer(f"Дата ({date.strftime('%Y-%m-%d')}) успешно выбрана ! ✅")

		# elif selected == NoneType :
