...
#--------------------------------------------------------------- Пробник
	...
	for ret in read:  
		...
		sent_message_ids.append(sent_message.message_id) # Сохраняем id сообщений
		#await msg.delete()
#---------------------------------------------------------------
#Ловим пятый ответ
...
#--------------------------------------------------------------- Пробник
		...
		sent_message_ids.clear()  # Очищаем список после удаления
#---------------------------------------------------------------
...
#--------------------------------------------------------------- Пробник
	read = await sqlite_db.sql_readlenina()
	for ret in read:  
		sent_message = await call.bot.send_photo(call.message.chat.id, ret[0], f'{ret[1]}\n{ret[2]}',reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton(text=f'Выбрать {ret[1]}', callback_data=f'service {ret[1]}')))
		sent_message_ids.append(sent_message.message_id) # Сохраняем id сообщений
#---------------------------------------------------------------
	...
	# 	await call.message.answer(f"{ret['title']} - {str(ret['price_min'])} руб.", reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton(text=f"Выбрать  {ret['title']}", callback_data=f"service {ret['id']}"))) #f"service {ret['title']}")))
