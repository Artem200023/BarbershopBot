...
class ScaleApp:
    def __init__(self, root):
        ...
        # Фрейм для устройств (левая панель)
        ...
        # Фрейм для добавления нового устройства
        ...
        # Фрейм для управления (правая часть)
        ...
        try:
            ...
            with pd.ExcelWriter(temp_filename, engine='openpyxl') as writer:
                for device_id, df in devices_data.items():
                    ...
                    result_df.to_excel(writer, sheet_name=f"Весы {device_id}", index=False)
            
            # Применяем форматирование
            self.excel_formatter.format_excel(temp_filename)
    ...
    def add_device(self):
        ...
        if device_id and ip and port:
            try:
                ...
                self.devices_listbox.insert(tk.END, f"{device_id} ({ip}:{port})")
    ...
    async def measure_single_device(self, device):
        ...
        while not self.stop_measurement_flag:
            try:
                ...
                self.log(f"Подключено к {device['id']} ({device['ip']}:{device['port']})")
                ...
                try:
                    while not self.stop_measurement_flag:
                        ...
                        try:
                            ...
                            if len(data) >= 14:  # Проверяем, что данных достаточно
                    ...
                    self.log(f"Ошибка связи с {device['id']}: {e}. Переподключение...")
                ...
                self.log(f"Не удалось подключиться к {device['id']}: {e}. Повтор через 3 сек...")
    ...
    def process_weight(self, device_id, weight):
        ...
        output = (f"{current_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                 f"Весы {device_id}: {weight}г ({status})\n"
    ...
    def start_single_measurement(self):
        ...
        self.stop_measurement_flag = False
        asyncio.create_task(self.measure_single_device(device))
